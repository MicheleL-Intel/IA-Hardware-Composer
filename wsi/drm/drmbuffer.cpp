...
void DrmBuffer::Initialize(const HwcBuffer& bo) {
  ...
  if (image_.handle_->meta_data_.usage_ == hwcomposer::kLayerCursor) {
    ...
    frame_buffer_format_ = image_.handle_->meta_data_.format_;
  }
...
const ResourceHandle& DrmBuffer::GetGpuResource(GpuDisplay egl_display,
    ...
    if ((image_.handle_->meta_data_.usage_ == kLayerVideo) && total_planes > 1) {
      if (total_planes == 2) {
        const EGLint attr_list_nv12[] = {
            ...
            static_cast<EGLint>(image_.handle_->meta_data_.format_),
  ...
  if (image_.image_ == VK_NULL_HANDLE) {
    ...
    VkFormat vk_format = NativeToVkFormat(image_.handle_->meta_data_.format_);
...
const MediaResourceHandle& DrmBuffer::GetMediaResource(MediaDisplay display,
  ...
  uint32_t rt_format = DrmFormatToRTFormat(image_.handle_->meta_data_.format_);
  uint32_t total_planes = image_.handle_->meta_data_.num_planes_;
  external.pixel_format = DrmFormatToVAFormat(image_.handle_->meta_data_.format_);
...
void DrmBuffer::Dump() {
  ...
  DUMPTRACE("Format: %4.4s", (char*)&image_.handle_->meta_data_.format_);
